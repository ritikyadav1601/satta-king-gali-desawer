...
import { getExtraGameYearChart } from "@/lib/extraGames";
...
export async function getTopGames(games) {
  ...
  const isPendingResult = (game) => String(game.second || "XX").toUpperCase() === "XX";
  const declared = byTime.filter((game) => !isPendingResult(game));
  const pending = byTime.filter(isPendingResult);
  const recentResult = declared.sort((a, b) => {
    const aUpdated = new Date(a.secondUpdatedAt || 0).valueOf() || 0;
    const bUpdated = new Date(b.secondUpdatedAt || 0).valueOf() || 0;
    return aUpdated - bUpdated || timeToMinutes(a.resultTime) - timeToMinutes(b.resultTime);
  }).at(-1);
  const upcoming = pending.find((game) => timeToMinutes(game.resultTime) > now) || pending[0];

  if (!recentResult) return pending.slice(0, 2);

  return [recentResult, upcoming || declared.find((game) => String(game._id) !== String(recentResult._id))].filter(Boolean);
}
...
export async function getYearChartRows(gameSlug, year) {
  // Games that only exist in the EXTRA_GAMES_MONGO_URI database read their chart from there.
  // Games that exist in Firebase / MONGODB_URI keep using the original logic below.
  if (process.env.EXTRA_GAMES_MONGO_URI || process.env.EXTRA_GAMES_MONGO_URO) {
    try {
      const mainGames = await getActiveGames("time");
      const inMainSource = mainGames.some((item) => normalizeKey(item.name) === normalizeKey(gameSlug));
      if (!inMainSource) {
        const extra = await getExtraGameYearChart(gameSlug, year);
        if (extra) return plain(extra);
      }
    } catch (error) {
      console.error("[extra-games] yearly chart read failed:", error.message);
    }
  }
  ...
  const games = await getActiveGames("time");
  const game = games.find((item) => resultGameKey(item.name) === resultGameKey(gameSlug));
  const rows = Array.from({ length: 31 }, (_, index) => ({
    Date: index + 1,
    JAN: "-",
    FEB: "-",
